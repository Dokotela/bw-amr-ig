import 'package:fhir_r4/fhir_r4.dart' show ValueSet;

/// http://bw.health.gov/fhir/amr/ValueSet/botswana-amr-organism-vs
/// (no version), from
/// fsh-generated/resources/ValueSet-botswana-amr-organism-vs.json.
/// Generated by tool/generate_artifacts.dart; do not edit.
final ValueSet botswanaAmrOrganismVs = ValueSet.fromJson(
  {
    'resourceType': 'ValueSet',
    'status': 'draft',
    'name': 'BotswanaAMROrganismVS',
    'id': 'botswana-amr-organism-vs',
    'title': 'Botswana AMR Organism ValueSet',
    'description':
        'Organisms identified in Botswana AMR surveillance: all '
        "descendants of SNOMED CT 'Organism' (410607006). This includes "
        'the combinations, groups and subspecies that laboratories report '
        "together — SNOMED writes a subspecies as 'ss.', a slash pair as "
        "'or', and a group as 'complex', so a search for the WHONET string "
        'as written finds nothing while the concept is there. See '
        'ConceptMap-retired-local-organisms-to-snomed for the 32 local '
        'codes this value set used to carry and what each one is.',
    'url': 'http://bw.health.gov/fhir/amr/ValueSet/botswana-amr-organism-vs',
    'compose': {
      'include': [
        {
          'system': 'http://snomed.info/sct',
          'filter': [
            {
              'property': 'concept',
              'op': 'is-a',
              'value': '410607006',
            },
          ],
        },
        {
          'system':
              'http://bw.health.gov/fhir/amr/CodeSystem/botswana-amr-local-organism-cs',
          'concept': [
            {
              'code': 'cronobacter-sakazakii-group',
              'display': 'Cronobacter sakazakii group',
            },
          ],
        },
      ],
    },
  },
);
